...
export const COMBO_PROMPT = `Ты кулинарный ассистент. Семья свободным текстом описала привычные блюда — часто уже готовыми парами («гречка с курицей», «рыба с картошкой», «котлеты с макаронами», «тушёные овощи с рагу из фасоли»). Твоя задача — РАЗЛОЖИТЬ эти блюда на компоненты и собрать ДВА списка РАЗНЫХ заготовок: белок отдельно, гарнир отдельно. Потом семья будет чередовать их между собой, поэтому:
- разбирай каждое составное блюдо на белковую часть и гарнир;
- собери полный набор РАЗНЫХ компонентов из всего описания, без повторов (если «курица» встречается в двух блюдах — она одна в списке proteins);
- чисто гарнирные упоминания (просто «гречка», «картошка на пару») — только в sides; чисто белковые («наггетсы», «стейк из лосося») — только в proteins.
...
- Списки — из РАЗНЫХ компонентов, без дублей.
- Все тексты на русском.`;

export function buildComboUserMsg(text) {
  return `Описание привычных блюд семьи:\n"""\n${text}\n"""\nРазложи блюда на компоненты и верни два списка РАЗНЫХ заготовок: sides (гарниры) и proteins (белок).`;
}

// Чистит ответ GPT: валидные title, категория — только из таксономии (иначе
// определяется по названию), ингредиенты чистятся как при импорте. Компоненты
// с одинаковым названием (без учёта регистра) схлопываются — блюда описаны
// парами, один и тот же гарнир/белок встречается в разных парах.
