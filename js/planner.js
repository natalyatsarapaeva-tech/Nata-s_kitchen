...
// Категории — отделы магазина, чтобы пройти список один раз и не возвращаться.
// Порядок в массиве = порядок групп в списке и на печати.
export const SHOP_CATEGORIES = [
  'vegetable', 'fruit', 'meat', 'fish', 'dairy', 'bakery', 'grocery', 'sauce', 'drink', 'other',
];
...
const CATEGORY_ORDER = Object.fromEntries(SHOP_CATEGORIES.map((c, i) => [c, i]));

// Категория из справочника (её проставляет GPT на странице 🥗) — таксономия
// там подробнее, чем отделы магазина, поэтому сводим её к отделам.
const DICT_CATEGORY_TO_SHOP = {
  vegetable: 'vegetable', herb: 'vegetable', mushroom: 'vegetable',
  fruit: 'fruit', berry: 'fruit',
  meat: 'meat', poultry: 'meat',
  fish: 'fish', seafood: 'fish',
  dairy: 'dairy', egg: 'dairy',
  bakery: 'bakery',
  grain: 'grocery', legume: 'grocery', nut: 'grocery', oil: 'grocery', sweet: 'grocery',
  sauce: 'sauce', spice: 'sauce',
  drink: 'drink',
  other: 'other',
};

// Определение отдела по названию продукта. ВАЖНО: \b в JS не дружит с
// кириллицей, поэтому ловим по префиксам слов, как в combo.js.
// Порядок = приоритет: сначала пары «прилагательное + продукт», которые иначе
// уехали бы не в тот отдел, потом отделы от специфичных к общим.
const SHOP_KEYWORDS = [
  // 1. Исключения: слово-маркер отдела внутри товара из другого отдела
  ['dairy', /сливочн[а-яё]*\s*масл|масл[а-яё]*\s*сливочн|топлён[а-яё]*\s*масл|топлен[а-яё]*\s*масл|(^|\s)гх[иы](\s|$)/],
  ['sauce', /томатн[а-яё]*\s*паст|паст[а-яё]*\s*томатн|рыбн[а-яё]*\s*соус|(чёрн|черн|душист|молот|красн|жгуч|кайенск|горошк)[а-яё]*\s*перц?е?ц|перец\s*(чёрн|черн|душист|молот|горошк|красн|чили)/],
  ['grocery', /оливков[а-яё]*\s*масл|масл[а-яё]*\s*оливков|(^|\s)оливк|маслин|кокосов[а-яё]*\s*молок|сгущ|сухое\s*молоко|молочн[а-яё]*\s*шоколад|панировочн|(^|\s)сухар|кукурузн[а-яё]*\s*(мук|крахмал)|сухофрукт|изюм|курага|чернослив|(^|\s)финик|цукат/],

  // фруктовый сок в пакете — напиток, а «сок лимона» — всё-таки лимон
  ['drink', /(апельсинов|яблочн|виноградн|ананасов|вишнёв|вишнев|гранатов|мультифрукт|томатн)[а-яё]*\s*сок|сок\s*(апельсинов|яблочн|виноградн|томатн)/],

  // 2. Белок
  ['fish', /рыб|лосос|сёмг|семг|форел|треск|тунец|тунц|скумбр|сельд|сайр|дорад|сибас|судак|минтай|палтус|камбал|горбуш|кет[аы]|нерк|(^|\s)хек|(^|\s)икр|кревет|мидии|мидий|кальмар|осьминог|гребешк|(^|\s)краб|устриц|морепродукт|анчоус|килька|шпрот/],
  ['meat', /куриц|курин|цыпл|индейк|индюш|(^|\s)утк|утин|гус[ья]|перепел|говя|свин|телят|баран|ягн[ёе]|кролик|(^|\s)мяс|фарш|бекон|ветчин|колбас|сосис|сардель|карбонад|грудк|бедр[оа]|голен|окорок|(^|\s)крыл|стейк|вырезк|рёбр|(^|\s)ребр|шпик|(^|\s)сал[оа](\s|$)|субпродукт|печень(\s|$)|печёнк|печенк|(^|\s)язык|котлет|тефтел|фрикадельк|наггетс|пельмен|шашлык|люля|бастурм|прошутто|хамон/],
  ['dairy', /молок|молочн|кефир|ряженк|простокваш|сметан|творог|творож|йогурт|сливк|(^|\s)сыр|моцарелл|пармезан|чеддер|гауд|фет[аы]|брынз|сулугуни|адыгейск|маскарпоне|рикотт|плавлен|яйц|яиц/],

  // 3. Свежее
  ['vegetable', /овощ|картоф|картошк|морков|(^|\s)лук|луковиц|порей|шалот|чеснок|черемш|капуст|брокколи|помидор|томат|огурц|огурец|(^|\s)перец|(^|\s)перц|кабачк|кабачок|цукини|баклажан|тыкв|свёкл|свекл|редис|редьк|(^|\s)репа|сельдерей|шпинат|салат|руккол|латук|зелен|зелён|укроп|петрушк|кинз|базилик|(^|\s)мят[аы]|розмарин|тимьян|щавел|спарж|артишок|стручков|горошек|кукуруз|(^|\s)гриб|шампиньон|вешенк|шиитаке|авокадо|имбир|(^|\s)чили|батат|топинамбур|пастернак|кольраби|проростк|микрозелен/],
  ['fruit', /яблок|груш|банан|апельсин|мандарин|лимон|лайм|грейпфрут|киви|виноград|(^|\s)слив[аы](\s|$)|персик|нектарин|абрикос|ананас|манго|хурм|гранат|инжир|дын[яи]|арбуз|ягод|клубник|земляник|малин|черник|голубик|ежевик|смородин|вишн|черешн|клюкв|брусник|облепих|крыжовник/],

  // 4. Полки
  ['bakery', /хлеб|батон|багет|булк|булочк|лаваш|(^|\s)пит[аы](\s|$)|тортиль|лепёшк|лепешк|круассан|бублик|бриош|чиабатт|фокачч|(^|\s)тест[оа]|дрожж/],
  ['grocery', /(^|\s)мук|(^|\s)круп|(^|\s)рис|греч|перловк|булгур|кускус|кус-кус|киноа|овсян|овся|хлопь|(^|\s)манк|пшен|пшён|полб|ячнев|макарон|спагетт|(^|\s)паст[аы](\s|$)|лапш|вермишел|пенне|фузилл|фарфалл|(^|\s)рожк|тальятел|феттуч|ньокк|чечевиц|фасол|(^|\s)нут|горох|бобы|бобов|(^|\s)маш(\s|$)|орех|миндал|кешью|фундук|фисташк|арахис|семечк|семена|кунжут|сахар|(^|\s)мёд(\s|$)|(^|\s)мед(\s|$)|варень|джем|шоколад|какао|ванил|разрыхлит|(^|\s)сод[аы]|желатин|агар|крахмал|(^|\s)масл|консерв|мюсли|гранол|печенье|вафл|бульонн|уксус|(^|\s)соль|чипсы/],
  ['sauce', /соус|кетчуп|майонез|горчиц|аджик|песто|терияки|соев|табаско|шрирач|(^|\s)хрен|васаби|специ|припра|паприк|куркум|карри|кориандр|(^|\s)зира|(^|\s)кумин|корица|гвоздик|мускат|лавров|орегано|прованск|хмели|сумах|шафран|(^|\s)тмин|(^|\s)анис|кардамон|бадьян/],
  ['drink', /(^|\s)вод[аы](\s|$)|минерал|(^|\s)сок(\s|$)|морс|компот|(^|\s)чай(\s|$)|кофе|(^|\s)вино(\s|$)|пиво|коньяк|(^|\s)ром(\s|$)|водк|шампанск|лимонад|квас|сироп/],
];

// Отдел магазина для позиции списка. Справочник заполняет GPT, и делает это
// нестабильно (курица уезжала то в meat, то в other, часть позиций вообще без
// категории) — поэтому сначала детерминированное правило по названию, и только
// потом категория из справочника.
export function classifyShoppingCategory(key, label = '', dictDoc = null) {
  for (const name of [key, label]) {
    const t = ' ' + String(name || '').toLowerCase().trim() + ' ';
    if (t.trim()) {
      for (const [cat, re] of SHOP_KEYWORDS) if (re.test(t)) return cat;
    }
  }
  return DICT_CATEGORY_TO_SHOP[dictDoc?.category] || 'other';
}

export function shopCategoryRank(category) {
  const i = CATEGORY_ORDER[category];
  return i == null ? CATEGORY_ORDER.other : i;
}
...
export function aggregateShopping(slots, recipesById, dict, profile = null) {
  ...
  for (const slot of Object.values(slots || {})) {
    ...
    for (const i of expandIngredients(r.ingredients)) {
      ...
      if (!item) {
        item = { key, label: i.n, category: 'other', dictCategory: d?.category || null, pcs: 0, grams: 0, lines: [], unitG };
        items.set(key, item);
      }
  ...
  // категорию считаем в конце: ярлык к этому моменту уже устоялся
  for (const item of items.values()) {
    item.category = classifyShoppingCategory(item.key, item.label, { category: item.dictCategory });
  }
  return [...items.values()].sort((a, b) =>
    shopCategoryRank(a.category) - shopCategoryRank(b.category)
